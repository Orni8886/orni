...
WHITELIST_IDS = [id.strip() for id in WHITELIST_IDS if id.strip().isdigit()]
...
async def on_message(message):
    ...
    if str(message.author.id) not in WHITELIST_IDS:
        await message.channel.send("Vous n'êtes pas autorisé à utiliser ce service.")
        return
